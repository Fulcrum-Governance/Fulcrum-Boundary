...
> `github.com/fulcrum-governance/fulcrum-boundary`, CLI binary `boundary`, target tag `v0.13.1`).
...
| **C10** | Distribution = **prebuilt one-command channels (Homebrew tap, container image, static + native-cgo archives with SHA256 manifests) from the tag-gated pipeline for `v0.10.1`+, plus `go install …/cmd/boundary@v0.13.1` from source** (`BND-CLAIM-DIST-001`, `delivered`). Still **no hosted-monitoring / cloud analysis**; the dashboard reads local artifacts only. | implied hosted monitoring / cloud analysis | §1.12, §7.4 |
...
|---|---|---|---|
| Install path | `brew install fulcrum-governance/tap/boundary` (static) or `go install github.com/fulcrum-governance/fulcrum-boundary/cmd/boundary@v0.13.1` (source) | repeatable release target | `docs/RELEASE_TRUTH_PUBLIC.md`; README |
...
- **Distribution (C10):** prebuilt channels (Homebrew tap, container image, archives + SHA256 manifests) ship from the tag-gated pipeline for `v0.10.1`+, plus `go install …/cmd/boundary@v0.13.1` from source (`BND-CLAIM-DIST-001`). **No hosted-monitoring**; the dashboard reads local artifacts only.
...
- Distribution copy reflects the release-target channels (Homebrew, container, archives, and `go install …@v0.13.1`) for `v0.10.1`+ and a `local-only` dashboard with no hosted monitoring. (C10)
