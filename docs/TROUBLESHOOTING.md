...
If `go version` reports an older release, `go install …@v0.13.0` can fail to
...
1. **Do not disable cgo.** `go install …@v0.13.0`, `go build`, and `make build`
   ...
   ```bash
   CGO_ENABLED=1 go install github.com/fulcrum-governance/fulcrum-boundary/cmd/boundary@v0.13.0
   ```
...
The `v0.13.0` release target also includes first-run environment diagnostics and
...
The `v0.13.0` release target can use `boundary doctor --report` to emit the same
