...
If `go version` reports an older release, `go install …@v0.12.0` can fail to
...
1. **Do not disable cgo.** `go install …@v0.12.0`, `go build`, and `make build`
   ...
   ```bash
   CGO_ENABLED=1 go install github.com/fulcrum-governance/fulcrum-boundary/cmd/boundary@v0.12.0
   ```
...
The published `v0.12.0` release also includes first-run environment diagnostics and
...
The published `v0.12.0` release can use `boundary doctor --report` to emit the same
