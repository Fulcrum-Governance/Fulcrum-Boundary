...
- **Released means in the `v0.12.0` tag.** The baseline below, all of Phase 0A,
  and Phase 1 (`boundary test`) are released in `v0.9.0` and exercised by tests
  and the two proof-lane demos. The current `v0.12.0` install includes the Phase 0A
...
- **Source-main is not release truth.** Phase 0B slices may land on `main`
  before the next tag, but they are not installable from `@v0.12.0` until a
...
## Phase 0A — Trust the Record / Evidence UX (shipped in `v0.8.0`, included in the current `v0.12.0` release)

> **In the `v0.8.0` release and included in the current `v0.12.0` release.** Everything in this
> section is released, exercised by tests, and reflected in the claims ledger.
> The current `v0.12.0` install includes these commands and record fields. Command and
> field reference:
...
in the current `v0.12.0` install.
...
[`docs/CLI_REFERENCE.md`](CLI_REFERENCE.md) §11), included in the current `v0.12.0`
install.
...
## Phase 0B — Diagnostics & first-impression clarity (shipped in the current `v0.12.0` release)

> **Shipped in the current `v0.12.0` release.** These Phase 0B slices landed on
> source `main` after `v0.9.0` and are now included in the `v0.12.0` install:
...
> current `v0.12.0` install includes it. The historical `@v0.8.0` install does not.
...
- The Baseline, Phase 0A, and Phase 1 sections describe behavior that is
  ...
  current `v0.12.0` install includes them.
