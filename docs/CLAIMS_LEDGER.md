...
| BND-CLAIM-DIST-002 | partial | Boundary's tag-gated release pipeline generates an SPDX SBOM for each static and native-cgo release archive and records GitHub build-provenance attestations for its release artifacts. | `tests/supplychain/wiring_test.go`, `docs/SUPPLY_CHAIN.md`, `docs/INSTALL.md` | Static-archive SBOM is verified via `goreleaser release --snapshot` and the cgo-archive SBOM command via local syft; build-provenance attestation and the cgo-archive SBOM are release-gated (live from the next tagged release) — gap BND-DIST-002. Distribution provenance only — distinct from decision-record signing (#134); `boundary version` does not prove provenance. |
