...
| BND-CLAIM-003 | partial | Boundary ships one production MCP adapter and six preview adapter packages with lifecycle maturity tracked per adapter. | Adapter package tests, CLI/CodeExec/gRPC/Webhook/A2A lifecycle and conformance tests, Managed Agents integration tests, `docs/ADAPTER_READINESS_MATRIX.md` | Use the maturity matrix. MCP is production; CLI, CodeExec, gRPC, Managed Agents, Webhook, and A2A remain preview until their named gaps are closed. |
