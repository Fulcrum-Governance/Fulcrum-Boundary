...
| BND-CLAIM-DIST-001 | delivered | Boundary publishes one-command install channels (Homebrew tap cask, prebuilt static and native-cgo archives with SHA256 checksum manifests, container image) from a tag-gated release pipeline. | `.goreleaser.yaml`, `.github/workflows/release.yml`, `docs/INSTALL.md` | Delivered by the first tag-gated binary release, `v0.10.1`; releases up to and including `v0.10.0` are source-only. Windows native-cgo archives are not published — a permanent stance (Windows is static-only by design); see docs/INSTALL.md. |
| BND-CLAIM-DIST-002 | delivered | Boundary's tag-gated release pipeline generates an SPDX SBOM for each static and native-cgo release archive and records GitHub build-provenance attestations for its release artifacts. | `tests/supplychain/wiring_test.go`, `docs/SUPPLY_CHAIN.md`, `docs/INSTALL.md` | First exercised and publicly verified at `v0.12.0`: attestations cover configured static archives and SBOMs, `SHA256SUMS`, and native-cgo archives and SBOMs. `SHA256SUMS-cgo` has no workflow attestation. Distribution provenance only — distinct from decision-record signing (#134); `boundary version` does not prove provenance. |
...
| BND-CLAIM-GH-020 | delivered | Boundary provides a Secure GitHub bypass-proof ladder (L0-L3) and a fail-closed deployment bypass-proof packet that records operator-owned live evidence and operator-attested deployment-topology denials. | `adapters/securegithub/bypass_ladder_test.go`, `adapters/securegithub/live_evidence_test.go`, `adapters/securegithub/bypass_packet_test.go`, `docs/secure-mcp/GITHUB_BYPASS_LADDER.md`, `docs/deployment/secure-github-bypass-proof-packet.md` | A passing L2 packet is an internal production-candidate gate only. It does not prove the deployment is bypass-proof or change Secure GitHub's preview status. |
| BND-CLAIM-GH-021 | delivered | Boundary provides a preview stateful local route to the official GitHub MCP endpoint for one configured untrusted source issue and one protected target repository and branch. | `adapters/securegithub/mcp_proxy_test.go`, `adapters/securegithub/mcp_evidence_test.go`, `internal/boundarycli/mcp_securegithub_test.go`, `docs/secure-mcp/GITHUB.md`, `docs/CLI_REFERENCE.md` | Only the two documented tools and configured source/target are governed. It does not semantically classify issue prose, close direct GitHub bypasses, or make Secure GitHub production. |
...
| BND-CLAIM-CMD-003 | delivered | Command Boundary classifies a whole command line by decomposing it into segments (`&&`, `\|\|`, `;`, `\|`, subshells, command substitution, `sh -c` payloads, redirect write targets, `find -exec` / `xargs` payloads) and takes the most restrictive segment's action as the line's verdict, flooring an undecomposable line at `require_approval`. | `internal/commandboundary/line_test.go`, `internal/redteam/command_smuggling_test.go`, `docs/command-boundary/CLASSIFY.md` | Decomposition is a model of shell syntax, not a shell. Heredocs, process substitution, `eval`, unbalanced quotes, and nesting past the depth cap come back undecomposable and escalate; they are never allowed, and no class claim is made about them. Whole-line decomposition applies where Boundary is handed a line (`boundary command classify` unsplit, redteam packs, the hook's Bash route), not to an already-split argv. |
| BND-CLAIM-HOOK-001 | delivered | Boundary decides Claude Code `PreToolUse` tool calls on the routes the hook is wired to intercept, and persists a canonical `DecisionRecordV1` for every decided verdict — allow, warn, require_approval, deny — before the decision reaches Claude Code; the hook never emits a permission grant, and a decision it cannot record is escalated rather than allowed unrecorded. | `tests/hookboundary/hook_decision_test.go`, `internal/hookboundary/decide_test.go`, `internal/hookboundary/decision_test.go`, `docs/integrations/CLAUDE_CODE_HOOK.md` | Routed-only, and preview-grade: an un-wired tool, an MCP tool, a tool a subprocess runs on its own, or a shell outside Claude Code is a bypass. Records are hash-verifiable for integrity — not authenticity, not proof the verdict was correct, not proof the action was executed or prevented. |
| BND-CLAIM-HOOK-002 | delivered | Boundary treats its own governance control surfaces (`.boundary/**`, `.claude/settings.json`, `.claude/settings.local.json`, `.claude/hooks/**`, the shipped hook wrappers, `hooks/hooks.json`) as outside the edit envelope and denies writes to those shapes on the command route as well as the edit route. | `internal/editboundary/classifier_test.go`, `internal/redteam/edit_control_surface_test.go`, `tests/hookboundary/hook_decision_test.go`, `docs/integrations/CLAUDE_CODE_HOOK.md`, `docs/edit-boundary/EDIT_TAXONOMY.md` | A set of path shapes, not an inventory of every way governance could be disabled. It does not prevent an agent from disabling governance by an unmodelled path, and it does not close direct shell access. |
| BND-CLAIM-HOOK-003 | delivered | Boundary ships a POSIX installer for the Claude Code surface that records what it installed in a receipt and reverses exactly what that receipt names on `--uninstall`, leaving a Homebrew-managed binary to Homebrew. | `tests/hookboundary/installer_test.go`, `integrations/claude-code/README.md`, `docs/integrations/CLAUDE_CODE_HOOK.md` | Reversal is scoped to what the receipt records. Installing the plugin wires the hook; it does not by itself govern tool calls the hook is not wired to intercept. |
| BND-CLAIM-WITNESS-001 | delivered | Fulcrum-Boundary ships fulcrum-verify-witnessed, a public, Apache-2.0, independently-buildable Go command that checks an exported witnessed-log bundle offline against locally supplied Fulcrum and witness Ed25519 public keys, with no Fulcrum code, account, credential, or network call required to build or run it. | `verify-witnessed/independence_test.go`, `verify-witnessed/main_test.go`, `verify-witnessed/README.md` | Closes the code/license gate only (ADR-041); does not establish that any witness is operated independently of Fulcrum — today's witnesses remain Fulcrum-operated, on separate infrastructure. |
